...
- **Song caching** does not happen in jukebox mode. Caching is triggered when local playback reaches the end of a track, but local playback (MPV and web) stays paused the whole time the server is the one actually playing audio - so that trigger never fires. This is expected: the server is already streaming the file directly, so the client re-downloading the same song just to cache it would be wasted bandwidth with no playback benefit unless you later switch back to local/web/MPV mode for that song.
