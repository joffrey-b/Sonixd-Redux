...
async function ensureRatingColumnVisible(window: Page) {
  ...
  // useRootClose + a focus-return side effect on the trigger) -- a live e2e
  // run caught the very next click (unrelated, elsewhere on the page)
  // stalling for a full 30s timeout immediately after this same sequence,
  // meaning the dropdown had not actually finished closing yet. Wait for
  // the popup itself to be gone rather than assuming the keypress landed.
  // Scoped to `window`, not `picker` -- the popup no longer renders inside
  // the trigger's own container (see the container-prop fix in
  // ListViewConfig.tsx), so it's not a DOM descendant of `picker` anymore.
